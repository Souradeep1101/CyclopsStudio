...
void main()
{
    ...
    // CRITICAL: This line must use u_ViewProjection
...
void main()
{
	...
	// ROBUST SAMPLING: Use a switch statement to satisfy all drivers.
	// This ensures the compiler doesn't optimize out the array.
	switch(int(v_TexIndex))
	{
		...
		// ... technically you can go to 31, but 16 is usually enough for a brush engine
		default: texColor *= texture(u_Textures[0], v_TexCoord * v_TilingFactor); break;
	}

	// Output final color
	// If texture is white (1,1,1,1), it just draws v_Color.
	// If v_Color is white, it draws the texture.
	color = texColor;
}