...
#include "Cyclops/Events/ApplicationEvent.h"
#include "Cyclops/Events/KeyEvent.h"
#include "Cyclops/Events/MouseEvent.h"
...
namespace Cyclops
{
	...
	void Engine::Init() 
	{
        ...
        glfwSwapInterval(1); // Change to 1 for VSync
        ...
        Cyclops::EnableGLDebugging();

        // Initialize the Batch Renderer (Allocates the GPU/CPU memory arena)
        Cyclops::Renderer2D::Init();
	...
	void Engine::Shutdown() 
    {
        ...
        // Free the Batch Renderer memory
        Renderer2D::Shutdown();
    ...
    void* Engine::GetGLProcAddress(const char* name)
    {
    ...
    {
        // [CRITICAL FIX] 
        // Force the OpenGL context to switch back to the Main Window 
        // before applying the VSync setting.
        if (s_Window)
        {
            glfwMakeContextCurrent(s_Window);
